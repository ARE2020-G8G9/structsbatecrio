...
def init_bact1(taux_opt1, ph, ph_opt1, temp, temp_opt1):
    return {'x' : 0, 'y' : 0, 'contenu' : 3, 'age' : 0, 'resistance' : 0, 'taux_croissance' : taux_de_croissance_effectif(taux_opt1, ph, ph_opt1, temp, temp_opt1)}

def init_bact2(taux_opt2, ph, ph_opt2, temp, temp_opt2):
    return {'x' : 0, 'y' : 0, 'contenu' : 3, 'age' : 0, 'resistance' : 0, 'taux_croissance' : taux_de_croissance_effectif(taux_opt2, ph, ph_opt2, temp, temp_opt2)}
...
def init_pos_bact(boite, bact_type):
    box = boite
    x, y = pos_alea(boite)
    box[x][y]['contenu'] = bact_type
    box[x][y]['taux_croissance'] = 1
    return box
...
def mort_bacterie(boite, x, y):
    box = boite
    box[x][y] = {"x" : x, "y": y, "contenu" : 1, "age" : -1, "resistance" : -1, "taux_de_croissance" : -1}
    return box

def deplacement_possible(boite, x, y, taille):
...
def deplacement(boite, x, y, new_x, new_y):
    ...
    if box[new_x][new_y]['contenu'] == 1:
        box[x][y]['age'] = 0
    box[new_x][new_y] = boite[x][y]
    box[x][y] = {"x" : 0, "y": 0, "contenu" : 0, "age" : -1, "resistance" : -1, "taux_de_croissance" : -1} #laisse un vide après s'être déplacée
    return box

def naissance(boite, x, y, new_x, new_y):
    box = boite
    bact_type = boite[x][y]['contenu']
    deplacement(boite, x, y, new_x, new_y)
    boite[x][y] = {"x" : 0, "y": 0, "contenu" : bact_type, "age" : 0, "resistance" : -1, "taux_de_croissance" : -1} #laisse une nouvelle bactérie après s'être déplacée
    return box
...
def structure_bacterio(iter_max):
    ...
    ph = 7
    box = init_boite(10)
    init_bact1(1, ph, 7, temp, 25)
    init_pos_bact(box, 3)
    cmpt = 0
    while cmpt < iter_max:
        for i in range(0, len(box)):
            for j in range(0, len(box[0])):
                if deplacement_possible(box, i, j, 1):
                    a, b = nourriture_plus_proche(box, i,j, 1)
                    naissance(box, i, j, a, b)
        plot_boite(box)
        cmpt+=1
        pause()
    return None
